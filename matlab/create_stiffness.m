...
%     lin_delta = 5*K_mfu + 10*C_uamb;
    ...
    for elem_index = 1:nb_elements_total  
        ...
        % negative solutions...
        tryp = 1;
        f_lin(n1) = f_lin(n1) + tryp*A1;
        f_lin(n2) = f_lin(n2) + tryp*A2;
        f_lin(n3) = f_lin(n3) + tryp*A3;
            ...
            node = boundary_nodes(i);
             if (i ~= 1)
                ...
                stiffness_matrix(a*nb_nodes+node,a*nb_nodes+node) = stiffness_matrix(a*nb_nodes+node,a*nb_nodes+node) + ...
                  ...
                  b_add = - rho* k * C_amb * (nodes(1,node)/3 + nodes(1,prev_node)/6);
                b(a*nb_nodes+node) = b(a*nb_nodes+node) + b_add;
             end
             if (i ~= length(boundary_nodes))
                ...
                b_add =  rho* k * C_amb * (- nodes(1,node)/3 - nodes(1,next_node)/6 );
                b(a*nb_nodes+node) = b(a*nb_nodes+node) + b_add;
